                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-body text-center bg-white-100 radius shadow-2xl">
                            <h2 class="mt-4">Chi tiết: Lớp {{ $attendance->subject->name }} </h2>
                            <h3>Ngày học: {{ $attendance->date->format('D d, m, Y') }}</h3>
                            <p class="text-bold"> số lượng học sinh: {{ $attendance->students->count() }} <i
                                    class="fas fa-users-class text-blue"></i></p>
                            <hr>
                            <div class="text-left">
                                <h2 class="mb-3 text-bold">Danh sách học sinh</h2>
                                <form action="{{ route('attendance.student.update', $attendance) }}" method="post">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-flush">
                                            ...
                                            <tr>
                                                <th scope="col" class="sort" data-sort="name">Tên học sinh</th>
                                                <th scope="col" class="sort" data-sort="action">Điểm danh</th>
                                            </tr>
                                            </thead>
                                            <tbody class="list">
                                                    ...
                                                    <td class="">
                                                        <div class="custom-control custom-radio d-inline mr-2">
                                                            ...
                                                            <label class="custom-control-label"
                                                                   for="radio-{{ $student->id }}-on">có mặt</label>
                                                        </div>
                                                        <div class="custom-control custom-radio d-inline">
                                                            ...
                                                            <label class="custom-control-label"
                                                                   for="radio-{{ $student->id }}-off">vắng mặt</label>
                                                        </div>
                                                    </td>
                                                </tr>
                                            @endforeach
                                    ...
                                    <div class="mt-4">
                                        <button type="submit" class="btn btn-primary btn-block radius">cập nhật</button>
                                    </div>
                                </form>
                            </div>
                        </div>
